...
public class EnglishLetterDecoderEncoder implements LetterDecoderEncoder {
    ...
    static public final int SUFFIX_LENGTH = 6;
    public static final int DASH_CHAR = 45;
    public static final int DASH_CODE = 27;

    public Integer encode(String string) {
        if (string.length() > SUFFIX_LENGTH)
            throw new SuffixToLongException("Suffix length should not be greater then " + SUFFIX_LENGTH);
        int result = 0;
        ...
        for (int i = string.length(); i < SUFFIX_LENGTH; i++) {
            result *= 28;
        }
        return result;
    }
    ...
    public int[] encodeToArray(String s) {
        ...
        while (s.length() > SUFFIX_LENGTH) {
            integers.add(encode(s.substring(0, SUFFIX_LENGTH)));
            s = s.substring(SUFFIX_LENGTH);
        }
